...
//		testingFile = "datasets/linear_regression_test.data";
	...
	/**
	 * train the model using linear regression with L2 regularizer The first
	 * column of the features should be set as all 1. because the first feature
	 * should perform as bias So we should not regularize the theta0
	 * 
	 ...
	 * 
	 * + No need to choose alpha (learning rate). + Don't need to iterate. -
	 * Need to compute (X^{T} * X)^{-1} (O(n^3)). - Slow if the number of
	 ...
	 * 
	 * If you want to set the lamdab (penalization coefficient), you can go to
	 * the Linear Regression constructor to update the value of lambda
	 * 
	 * In addition, you can update the training path and testing path in the
	 * same way.
	 * 
	 * @param args
	 */
